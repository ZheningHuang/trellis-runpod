# TRELLIS RunPod serverless worker — proven build from camenduru/trellis-tost (CUDA 12.6 +
# torch 2.5.1 + flash-attn + kaolin/nvdiffrast/diso/spconv), with TRELLIS-image-large weights
# BAKED IN at /content/model. Only the handler is ours (worker_runpod.py = clean image→GLB).
# Deploy via RunPod's GitHub integration (RunPod builds this); anyone can deploy it with their key.
FROM ubuntu:22.04
